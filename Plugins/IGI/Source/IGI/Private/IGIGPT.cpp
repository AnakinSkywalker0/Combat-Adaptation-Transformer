...
namespace
{
    ...
    // RTX 4060 laptops typically have 8 GB VRAM; keep a safe budget to avoid backend aborts.
    constexpr std::size_t VRAM_BUDGET_RECOMMENDATION{ 4096 };
    ...
    // Prefer the D3D12 backend inside UE to avoid CUDA backend aborts in-editor.
    const nvigi::PluginID& SelectedGPTBackendId()
    {
        return nvigi::plugin::gpt::ggml::d3d12::kId;
    }
}
...
public:
        ...
        : IGIModulePtr(IGIModule)
    {
        ...
        Result = IGIModulePtr->LoadIGIFeature(SelectedGPTBackendId(), &GPTInterface, nullptr);
        if (Result != nvigi::kResultOk || GPTInterface == nullptr)
        {
            UE_LOG(LogIGISDK, Error, TEXT("Unable to load gpt.ggml.d3d12 feature: %s"), *GetIGIStatusString(Result));
            GPTInstance = nullptr;
            return;
        }
        ...
        if (Result != nvigi::kResultOk)
        {
            UE_LOG(LogIGISDK, Error, TEXT("Unable to create gpt.ggml.d3d12 instance: %s"), *GetIGIStatusString(Result));
            GPTInstance = nullptr;
        }
    }
    ...
    virtual ~Impl()
    {
        ...
        if (IGIModulePtr)
        {
            IGIModulePtr->UnloadIGIFeature(SelectedGPTBackendId(), GPTInterface);
    ...
    FString Evaluate(const FString& SystemPrompt, const FString& UserPrompt, const FString& AssistantPrompt)
    {
        if (GPTInstance == nullptr)
        {
            UE_LOG(LogIGISDK, Error, TEXT("GPT instance is null; skipping inference"));
            return FString();
        }
